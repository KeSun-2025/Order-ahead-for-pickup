...
function Throughput(AS)
    ...
    Nbar = 10; %Trancation of state space
    lmdv = 0.5:0.01:2.5;
    USb = V - (1:nbar)*c/mu;
    ...
    for l = 1 : length(lmdv)
        ...
        THr(l) = 0;
        for n_r = Nbar : -1 : nbar
            [betal betah THe  TH_A TH_S] = HeteroDist(lmdv(l),n_r);
            if THe > THr(l)
                THr(l) = THe;
                TH_A_r(l) = TH_A;
                TH_S_r(l) = TH_S;
    ...
    plot(lmdv,THr,'r:','LineWidth',3)
    hold on



    function [betal betah THe TH_A TH_S] = HeteroDist(lmd,N)
        ...
        for lmd_A = gamma*lmd : -step : 0 
            for lmd_S = 0 : step : gamma*lmd-lmd_A
                ...
                elseif lmd_A > 0 && lmd_S > 0 && abs(lmd_A + lmd_S - gamma*lmd) < step/100
                ...
                elseif abs(lmd_A - gamma*lmd) < step/100 && lmd_S == 0
                    UA_a = sum(Ubar_a.*Pi_lmd);
                    UA_b = sum(Ubar_b.*Pi_lmd);
                    US_b = sum(USb.*Pi_lmd(1:nbar))-c/b; 
                    if UA_b >= US_b && UA_a >= 0
