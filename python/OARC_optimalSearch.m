...
[THopt_all, QEopt_all, N1opt_all, N2opt_all] = OptThre(mu);
writematrix(QEopt_all, 'QE_OARC_opt2.csv');   
writematrix(N1opt_all, 'NR_OARC_opt2.csv');    
writematrix(N2opt_all, 'NC_OARC_opt2.csv');   

function [THopt, qeopt, N1opt, N2opt] = OptThre(mu)
...
THopt = zeros(length(Vv),length(gammav),length(betav),length(lmdv));
qeopt = zeros(length(Vv),length(gammav),length(betav),length(lmdv));
N1opt = zeros(length(Vv),length(gammav),length(betav),length(lmdv));
N2opt = zeros(length(Vv),length(gammav),length(betav),length(lmdv));
